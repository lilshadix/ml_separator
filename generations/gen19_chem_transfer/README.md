...
| decision | `decisions/D00_corpus_feasibility.md` (corpus feasibility), `D01_transfer_signal.md` (pre-seal baseline difficulty), `D02_factorization.md` (H1 / H1b / H4 and the ladder; regenerated by `scripts/g19_build_report.py`), `D03_actinide_transfer.md` (H3, F4 under both readings), `D04_mechanism_experts.md` and `D05_uncertainty.md` (the ladder runner; M3–M7 not run), `D06_process_integration.md` (the exploratory process run COMPLETED 2026-09-24 on the deployed M0 WITHOUT-actinide fit, every number labelled transfer-unsupported; verdict null -- no recipe reaches P(both & feasible) >= 0.5, S2(d) 0.35-0.65 < 0.80 in every cell, F5 does not hold), `D07_confirmation_not_run.md` (the confirmation run and the single V6 run were NOT RUN; cost, blockers, launch command) — all in the brief §29 format |
...
| D–H — models, uncertainty, process | **ladder RUN, exit 0** (`scripts/g19_run_ladder.py`): every step M3–M7 is `not_run` because neither M1 nor M2 is retained (POST-HOC addendum 3 item 3), `retained_final` **M0 (= B5)**, `stop_rule` false, and the ladder's own 40 h budget is **unconsumed** — `budget.used_hours` 0.0 of 40.0, `exhausted` false (`evaluation/ladder/decisions/{ladder,wall_clock}.json`, `tables/ladder_decisions.csv`, D04, D05). The ledger also records each invocation's `process_seconds` beside the budget's `seconds`, so an invocation that ran and skipped every step is distinguishable from one that never ran, and states the discovery run's 76.5955 h against its registered 60 h (task X finding V-L4). **The section 8 power check has run** (`scripts/g19_run_power.py`; `evaluation/power/`, `tables/power_kappa.csv`, `tables/reliability_before_correlation.csv`, `tables/power_per_unit_mae.csv`). **H3 (section 11) has RUN and is SCORED** (`scripts/g19_run_h3.py`; `evaluation/h3/`, D03): **10 of the 12** planned contrast record sets are complete and scored — every WITHOUT and ACT_PERMUTED leg of B5 and B6 on V5 and V2, and the WITHOUT legs on V1 — and the two `ACT_PERMUTED@V1` legs are `INCOMPLETE_GUARD_FAILURE` (25 of 39 folds each), not `NOT_RUN`: the 20 h cap was not reached (5.78 h used) and the legs stopped on a deterministic section 2 isolation-check failure at fold `pub_97510df3a0`, which `scripts/g19_h3_guard_diagnosis.py` traces to the near-duplicate **value** comparison reading the ACT_PERMUTED arm's permuted `log_D`; the fold is not forced. Completeness is a property of the contrast record set, not of the design (`h3.CONTRAST_UNIT_RULE`, a reading of addendum 4 item 2 for which an addendum is REQUESTED). Verdicts **B5 UNDECIDED, B6 UNDECIDED** (no registered power check; 20 H3 contrasts owe one). **F4 (negative actinide transfer) HOLDS** on the conservative reading — either registered 95 % interval on any registered design: the BCa interval of WITHOUT vs WITH on V2 excludes 0 ([+0.0031, +0.0783], Δ +0.0264) while the percentile interval does not, so the percentile-only reading does not hold and the two disagree on V2 (`h3_f4.json → failure`, `failure_percentile`, `failure_bca`). **Process step (§14): the exploratory run has COMPLETED (2026-09-24; `scripts/g19_predict_process_inputs.py` then `scripts/g19_run_process.py --exploratory --n-lhs 300`, exit 0; re-run under the TASK X fix-round code the same day, byte-comparable).** The deployment producer fits M0 (= B5) frozen at the MODE configuration of the 28 V5-primary selection-half records (`depth8_l23`, median 2,992 iterations) on the WITHOUT-actinide rows (addendum 5 item 3; a WITH table beside it), section 12 split-conformal intervals on the addendum-1 inner design (q95 WITHOUT 2.019), section 13 support ({'CONDITION_EXTRAPOLATION': 246, 'INTERPOLATION': 204}), and the residual correlation rho = 0.669 -- an Ln-Ln STAND-IN (2836 pairs; 0 Pr/Nd pairs among the calibration rows), flagged. Monte Carlo: 64 registered draws x 300 LHS operating points (below gen18's default 1,000 for the wall-clock cap; recorded), 1067 failed cascades. **Every output is labelled transfer-unsupported** (S1 UNDECIDED; `evaluation/process/`, `tables/process_*.csv`, F14 / F15, `decisions/D06_process_integration.md`): winners c100, c214 (support rank 2, INTERPOLATION cells only), P(both) at most 0.125, P(feasible) = 0 for every winner (third-phase / high-loading flags on most converged draws: the placeholder feed of 75 mM Nd exceeds the sourced LOC), S2(d) 0.35-0.65 in every cell (FAIL < 0.80), F5 does not hold; D06 verdict **null**; no Gen19 process recommendation; gen18 keeps its B1 lookup default. Then the confirmation run — see the next rows |
| report, figures 7–13 | **RUN 2026-09-24** (`scripts/g19_make_figures.py` and `scripts/g19_build_report.py`, both exit 0; `manifests/g19_make_figures.json`, `manifests/g19_build_report.json`). Figures F07–F11 and F13 written (`figures/`, data in `figures/data/`, index `evaluation/figures/figures_index.json`; F10 / F11 from M2 and M1 refits reproduced from their verified records, 56/56 state digests equal); **F12 and the V6 panel of F13 skipped — "confirmation run not run"**. Redrawn 2026-09-24 afternoon with `--no-embedding-refits` after the F13 direction gate adopted the registered `>= 0.3 − 1e-9` tolerance (6,679 pairs, accuracy 0.95673, as the scorer's S1(c) block counts; the morning run had 6,667 / 0.956952): the `figures` stage is re-registered from the tree (`manifests/digest_registry.json` → `stages.figures.code_digest` `f6488cc6…`, superseding `b1cb4cc2…`, against which the morning run's code digest `8830da5a…` was unregistered), and the `report` stage likewise (`a44acc64…`). [`GEN19_REPORT.md`](GEN19_REPORT.md): the ten questions of brief §34 answered in one sentence each ("Under deliberately hidden chemistry, the system can / cannot / has not been shown to …"), evidence tables with regime columns and both comparators (B0 and the cheapest sensible alternative), the FLAT floor for logSF, item-by-item R19, TOST, BH-adjusted p beside raw p, the reduced-sensitivity label; the S1 / S2 / F1–F6 verdict table; what was not run and why (addenda 1–8, the stop rule, the ladder, the budget demotion, the power debt, the confirmation / V6 omission with its cost and blockers); deviations and the addenda list; reproducibility (git HEAD, prereg and addenda digests, dataset hash, every registry entry, manifests). [`SUMMARY.md`](SUMMARY.md) is the one-page version; `tables/claims.json` the machine-readable one; `tables/report_numbers.csv` holds every printed number with its source path and key, re-resolved after writing (`manifests/g19_build_report.json` → `verification`; a number that does not re-resolve makes the script exit 3). **First screen of both documents: the confirmation run and the single Pr/Nd (V6) test were NOT RUN, so every full R19 verdict of a learned-arm contrast, and therefore S1 and S2, is UNDECIDED (S1(d), F3 and F4 stand as printed).** The deployed predictor is M0 (alias B5 in every record and table) and, for lanthanide prediction, its WITHOUT-actinide fit (§11, addendum 5 item 3); F4 HOLDS under the conservative reading and is stated under both registered readings; F6 is UNDECIDED (S1(a) is UNDECIDED, not failed). `tests/test_report.py` (17 tests, synthetic inputs only) |
| confirmation run (§15) and V6 | **NOT RUN — runner READY** ([`decisions/D07_confirmation_not_run.md`](decisions/D07_confirmation_not_run.md); `decisions/CONFIRMATION_PLAN.md` status block 2026-09-24). Four verification rounds of the once-only runner `scripts/g19_run_confirmation.py` each found blocking defects and the orchestrator stopped the track to deliver the report; the fifth round then fixed every blocker (missing M1 record = one INCOMPLETE M2 fold; a spent run refused; C4's two legs on distinct record paths with the section 11 permutation, the frozen refit and the addendum 5 item 2 guard; the V6 configuration reading of addenda 7-8 recorded in every V6 record) and REHEARSED `main()` end to end on a synthetic corpus (`tests/test_confirmation_rehearsal.py`), and its verification's seven further findings were fixed (resume unit = the fold's whole write set, atomic record writes, every worker probed, the M2<-M1 / ACT_PERMUTED<-WITH pairings audited, the V6 builder's converse scope check). **All 16 `RUN_STAGES` implemented**; stage re-registered from the tree (`6329bc4da416…`); `--dry-run` and `--check-only` with the real store pass (lock `first_run`). Planned cost of the core run: **132.23 h serial, 67.47 h wall clock at 2 workers**, 85 jobs, 3,418 folds (`manifests/g19_run_confirmation_dry_run.json` → `dry_run`; the V6 learned arms at their tuned cost). The withheld seeds stay committed (`manifests/confirmation_seeds_sha256.txt`, `65e8ae8c…5f82`) and unused; `evaluation/confirmation/` does not exist; nothing has ever run on V6 or on the confirmation half. Consequences until it runs: C1–C4 UNDECIDED (R19 item 4 NOT_EVALUATED), S2(a)–(c) NOT_RUN, brief §34 Q3 not answered, S1 UNDECIDED; the process layer ran only in exploratory mode, labelled transfer-unsupported |
...
the five withheld seeds have never been read (`decisions/decisions.json` → `guard_counters`:
...
1. [`SUMMARY.md`](SUMMARY.md) — one page: the ten answers of brief §34, the verdict table, and the statement that the confirmation run and the single Pr/Nd test were not run (every full R19 verdict of a learned-arm contrast, hence S1 and S2, UNDECIDED; S1(d), F3 and F4 stand as printed); then [`GEN19_REPORT.md`](GEN19_REPORT.md), the full report, every number cited by file and key (`tables/report_numbers.csv`); then [`decisions/D07_confirmation_not_run.md`](decisions/D07_confirmation_not_run.md) for what a future session must do to run the confirmation.
2. `GEN19_CLAUDE_CODE_INSTRUCTIONS.md` — the brief (the task, non-negotiable rules, validation designs V0–V7).
3. [`DATA_AUDIT.md`](DATA_AUDIT.md) — Phase A, items 1–14: what the corpus is, its hashes, counts,
...
4. [`FEASIBILITY.md`](FEASIBILITY.md) — Phase B, questions 1–13, the V0–V7 support table, the
   feasibility of the §30 actinide / §31 architecture / §32 priors questions, the §19 Pr/Nd table.
5. [`decisions/`](decisions/) — [`D00_corpus_feasibility.md`](decisions/D00_corpus_feasibility.md) and
...
6. [`preregistration_draft.md`](preregistration_draft.md) — the draft registration of the Phase C+
