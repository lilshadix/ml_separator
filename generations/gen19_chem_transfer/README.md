...
| confirmation run (§15) and V6 | **NOT RUN** ([`decisions/D07_confirmation_not_run.md`](decisions/D07_confirmation_not_run.md); `decisions/CONFIRMATION_PLAN.md` status block 2026-09-24). Four verification rounds of the once-only runner `scripts/g19_run_confirmation.py` each found blocking defects (the last: a missing M1 record aborted the run, a spent run was resumable, C4's two legs shared one record path); two `RUN_STAGES` — `v6_frozen_configurations` and `c4_act_permuted_training_transform` — are NOT IMPLEMENTED and the runner refuses at the first before a manifest, a fold file or the once-only lock is written; the orchestrator stopped the track to deliver the report. Planned cost of the core run: 110.24 h serial, 56.25 h wall clock at 2 workers, 85 jobs, 3,418 folds (`manifests/g19_run_confirmation_dry_run.json` → `dry_run`, `--dry-run` exit 0, no seed read). The withheld seeds stay committed (`manifests/confirmation_seeds_sha256.txt`, `65e8ae8c…5f82`) and unused; `evaluation/confirmation/` does not exist; nothing has ever run on V6 or on the confirmation half. Consequences: C1–C4 UNDECIDED (R19 item 4 NOT_EVALUATED), S2(a)–(c) NOT_RUN, brief §34 Q3 not answered, S1 UNDECIDED; the process layer may run only in exploratory mode, labelled transfer-unsupported |
