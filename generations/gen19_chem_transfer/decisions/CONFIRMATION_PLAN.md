...
**Status (2026-09-25): RUNNING — launched once, resumed three times; seed indices i0, i1 and i2 complete, i3 fitting.** The run was launched on 2026-09-24 and the once-only lock is **SPENT** (`evaluation/confirmation/decisions/run_started.json`, `started_utc` 2026-09-23T23:37:09Z), at the frozen plan `1a6e1844ad37…` and code digest `6329bc4da416…`, against the seed commitment `65e8ae8ceb8e…` (5 seeds, values withheld and still unread by any agent). **Nothing is scored yet:** `evaluation/confirmation/decisions/confirmation.json` does not exist, so every registered verdict stays UNDECIDED (R19 item 4 NOT_EVALUATED) and S2 with brief §34 Q3 stay NOT ANSWERED — exactly as the report prints them. Measured cost: seed index i2 took 7.55 h wall at 2 workers, so i3–i5 need ≈23 h. Progress per seed index, the three interruptions (each a Claude Code process exit, none of which lost a fitted fold) and the resume command are in `evaluation/confirmation/RUN_STATUS.md`. The 2026-09-24 status below is superseded only as to *whether* the run has started; everything it records about the runner's readiness still holds.

**Status (2026-09-24, after the fix round): NOT RUN — runner READY.** The orchestrator stopped the track on 2026-09-24 to deliver the report after four verification rounds of the once-only runner had each found blocking defects (the last: a missing M1 record would abort the run rather than one fold, a spent run was resumable, C4's two legs shared one record path, two `RUN_STAGES` NOT IMPLEMENTED). The fifth round (`gen19-finish-runner-and-process`, 2026-09-24) fixed every blocker and rehearsed `main()` end to end on a synthetic corpus (`tests/test_confirmation_rehearsal.py`), and the verification of that round found seven further minor defects, all fixed (`manifests/digest_registry.json → code_changes`, 'TASK X fix round'): the resume unit is now the fold's whole write set, record writes are atomic, every pool worker serves a pre-flight probe, the M2←M1 and ACT_PERMUTED←WITH pairings are audited at dispatch and at assembly, and the V6 builder asserts the converse of its scope check. **All 16 stages of `RUN_STAGES` are implemented**; `--dry-run` and `--check-only` (real store, gates only) pass at the re-registered code digest `6329bc4da416…`. The run itself has still not been started: `evaluation/confirmation/` does not exist, the once-only lock is unspent, and the launch is the orchestrator's (D07 'Next action'). Nothing below was executed: `evaluation/confirmation/` does not exist, the withheld seeds were never read (commitment `manifests/confirmation_seeds_sha256.txt`), every registered verdict stays UNDECIDED (R19 item 4 NOT_EVALUATED), S2 and brief §34 Q3 are NOT ANSWERED. The record of this decision and what a future session must do is `decisions/D07_confirmation_not_run.md`; the dry-run and gate captures are `manifests/g19_run_confirmation_dry_run.json`.
...
| the single V6 run (S2) | 8.5 h | **30.5 h** (learned arms at their TUNED cost: M1 tunes per §7 on each V6 fold with the V5 inner design, M2 takes M1's same-fold record — `confirmation.READINGS['v6_frozen_configurations']`; the 8.5 h assumed a carried per-fold configuration that no text names) | §3.4 runs V6 once; addenda 7 item 2, 8 item 1 |
...
| **total** | ≈ 174 h serial, ≈ 89 h wall | **132.23 h serial, 67.47 h wall at 2 workers** (85 jobs, 3,418 folds, 85 distinct record directories; of the serial total, the closed-form comparator and yardstick lines are 2.55 h and C3's B6r0 line 1.48 h — `dry_run.by_purpose_serial_hours`; the earlier 110.24 / 56.25 h priced the V6 learned arms at a frozen-refit cost the runner does not implement and is superseded; ±30 % band: 93–172 h serial, 47–88 h wall) | `manifests/g19_run_confirmation_dry_run.json → dry_run.serial_hours = 132.2321, dry_run.wall_hours_2_workers = 67.4653` (the captured stdout of `scripts/g19_run_confirmation.py --dry-run` at git HEAD `c20331c41c928b706ca0ae81d794a3bed379d0ed`, 2026-09-24) |
...
| V6: M1 + M2 at the frozen configuration (0.2785 × 988.4) — SUPERSEDED: the runner tunes M1 per §7 on each V6 fold and M2 takes M1's same-fold record (`READINGS['v6_frozen_configurations']`), so the V6 learned arms are priced at their TUNED unit cost; the live figure is `dry_run.by_purpose_serial_hours['the single V6 run (S2)']` = 30.48 h serial in §0's table | 988.4 (tuned) | 13 × 5 = 65 | 30.5 | 15.5 |
| V6: B8 yardstick (0.2785 × 699.6) — SUPERSEDED likewise: B8 has no tuning step and is priced at its measured unit cost inside the same §0 line | 194.8 | 65 | 3.5 | 1.8 |
